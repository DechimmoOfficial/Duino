...
class Settings:
    ...
    BAUDRATE = 115200
    DATA_DIR = "Duino-Coin Russia AVR Miner " + str(VER)
    SEPARATOR = ","
    ENCODING = "utf-8"
    try:
...
if not Path(Settings.DATA_DIR + '/Translations.json').is_file():
    url = ('https://drive.google.com/uc?export=download&id=1SnrZIzAJhtVim4jHD54sBmCsn9z_uXH_')
...
def periodic_report(start_time, end_time, shares,
    ...
    pretty_print("sys0",
                 ...
                 + str(int(hashrate)) + " хэшей/с" + get_string('report_body5')
